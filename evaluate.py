...
def compute_score():
    with torch.no_grad():
        ...
        for k in range(eval_iters):
            ...
            # predicted_indices, target_indices = predictions.view(-1), Y.view(-1)
            itos = meta['itos']
            predicted_sentences = [
                ''.join([itos[idx.item()] for idx in sentence]) for sentence in predictions
            ]
            target_sentences = [
                [''.join([itos[idx.item()] for idx in sentence])] for sentence in Y
            ]
